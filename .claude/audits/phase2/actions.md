...
## Phase D — Post Editor & AI Co-Writing ✅ COMPLETE (2026-02-26)

*Depends on Phase B (specifically #7 PostDraft model, #11 ChatSession extension, #12 platform config).*

- [x] **#20 — Create Post Editor Agent (Agent 6)** (`agents/postEditor.ts` CREATE): New Mastra agent `postEditorAgent` with `id: 'post-editor'`, model `google('gemini-2.5-flash')`. System prompt covers four roles: WRITE, EDIT, REFINE, EXPAND/COMPRESS. Hidden `<!--POST_CONTENT { "action", "content", "charCount", "explanation" } POST_CONTENT-->` block pattern for content extraction. Helpers: `parsePostContent()`, `stripPostContentBlock()`. `runPostEditor()` with quota pre-flight check, `findOrCreateEditorSession()`, sliding-window chat history (`applyHistorySlidingWindow`), and `buildEditorPrompt()` including persona voice + content brief + platform rules. ✅ DONE
- [x] **#21 — Register post editor agent in Mastra** (`agents/mastra.ts` MODIFY): Imported `postEditorAgent` from `./postEditor` and added to the Mastra instance's `agents` map alongside the existing 5 agents. ✅ DONE
- [x] **#22 — Create drafts API routes** (`routes/drafts.ts` CREATE, `index.ts` MODIFY): Full CRUD + chat: `POST /api/drafts`, `GET /api/drafts` (paginated, optional `?status=`), `GET /api/drafts/:id`, `PATCH /api/drafts/:id`, `DELETE /api/drafts/:id`, `POST /api/drafts/:id/chat` (calls `runPostEditor()`, strips POST_CONTENT before DB persist, returns `{ reply, postContent?, charCount?, changeExplanation? }`, 429 quota handling), `GET /api/drafts/:id/chat/history`, `POST /api/drafts/:id/publish` (sets status + upserts `SuggestionFeedback` with `action: 'published', rating: 'loved'`). Special `__INIT__` message rewrite. AI timeout registered in `index.ts` for `/:id/chat`. ✅ DONE
- [x] **#23 — Create draft service layer** (`services/draftService.ts` CREATE): `createDraft()`, `getDraft()`, `listDrafts()` (paginated), `updateDraft()` (auto-appends `contentHistory` + updates `charCount`), `applyAiContent()` (editedBy: 'ai'), `deleteDraft()`, `publishDraft()`. ✅ DONE
- [x] **#24 — Build editor page** (`app/dashboard/editor/page.tsx` CREATE): Reads `?draftId=` from URL. Loads draft + chat history in parallel via `Promise.all`. Dual-pane layout: left = `PostEditorPane`, right = `EditorChatPane` (360px fixed). Toolbar with editable title (auto-saves on blur), platform selector, Publish button + status badge. `handleApplyEdit()` applies AI content to editor and auto-saves. Auto-sends `__INIT__` when draft has empty content AND no chat history. Save, Mark Ready, Publish all wired. Back link → `/dashboard`. ✅ DONE
- [x] **#25 — Build PostEditorPane component** (`components/editor/PostEditorPane.tsx` CREATE): Controlled textarea with real-time char count. Platform-aware: LinkedIn 3,000-char limit; Twitter 280-char limit. Color-coded counter (gray → amber at 90% → red at 100%). When Twitter and content > 280 chars, `splitIntoTweets()` renders numbered tweet preview boxes with per-tweet counts. Save Draft, Mark Ready, Copy to Clipboard buttons. ✅ DONE
- [x] **#26 — Build EditorChatPane component** (`components/editor/EditorChatPane.tsx` CREATE): Chat interface scoped to a draft, calls `POST /api/drafts/:id/chat`. Quick-start prompt chips in empty state. `autoInit` prop: auto-sends `__INIT__` on mount (guarded by `autoInitFiredRef` against StrictMode double-fire); shows custom "Generating first draft…" loading state. Typing indicator (3 bouncing dots). Apply Edit / Discard banner when AI returns `postContent`. `onApplyEdit(content)` callback to push to PostEditorPane. Enter to send, Shift+Enter for newline. `__INIT__` messages hidden from visible chat history. ✅ DONE
- [x] **#27 — Add [Write This Post] button to SuggestionCard** (`components/suggestions/SuggestionCard.tsx` MODIFY): Added "✏️ Write This Post" button at card footer (above feedback section). On click: calls `draftsApi.create()` with full content brief (topic, angle, format, hook, postPointers, callToAction, seoKeywords), passes `sourceSuggestionSetId` + `sourceSuggestionIndex`, then `router.push('/dashboard/editor?draftId=...')`. Loading state "Creating draft…" while in-flight; error display on failure. ✅ DONE
- [x] **#28 — Add drafts API client** (`apps/web/src/lib/api.ts` MODIFY): Added `draftsApi` with `create()`, `list()`, `get()`, `update()`, `delete()`, `chat()` (uses `requestAI` — 180s timeout), `getHistory()`, `publish()`. Fixed duplicate `IPaginatedResponse` import (removed from second import block). Exports `IPostDraft`, `IDraftBrief`, `DraftStatus`, `DraftPlatform` types. ✅ DONE

---

## Phase E — Post Library ✅ COMPLETE (2026-02-26)

*Depends on Phase D (#22 drafts routes, #23 draft service, #28 API client).*

- [x] **#29 — Build post library page** (`app/dashboard/posts/page.tsx` CREATE): Page at `/dashboard/posts`. Status tab bar: All / Drafting / Ready / Published with counts displayed on the active tab. Paginated list of `PostListItem` rows (`PAGE_LIMIT = 10`). `fetchDrafts(tab, page)` via `draftsApi.list({ status, page })` — resets to page 1 on tab change. Pagination: ← Previous / Next → buttons with "Page X of Y (Z total)". Per-tab empty states with relevant copy. [+ New Post] creates a blank draft via `draftsApi.create()` then navigates to `/dashboard/editor?draftId=...`. Optimistic `handleStatusChange()` and `handleDelete()` callbacks. ✅ DONE
- [x] **#30 — Build PostListItem component** (`components/posts/PostListItem.tsx` CREATE): Row showing title (falls back to first line of content), platform badge (LinkedIn indigo / Twitter sky), status chip (Drafting gray / Ready blue / Published green), char count, `timeAgo()` timestamp (edited or published). Action buttons by status — Drafting: [✏️ Edit] [✅ Mark Ready] [🗑️ Delete]; Ready: [✏️ Edit] [📋 Copy] [🚀 Publish]; Published: [👁️ View] [📋 Copy]. All actions have loading states and inline error display. Optimistic updates via `onStatusChange` / `onDelete` callbacks. ✅ DONE
- [x] **#31 — Implement "Feed to Persona" publish flow** (`routes/drafts.ts` MODIFY, `services/draftService.ts` MODIFY): `publishDraft()` in draftService sets `status='published'` + `publishedAt`. Route handler upserts `SuggestionFeedback` with `action:'published', rating:'loved'` for source suggestion (strong positive signal). New `feedPublishedDraftToPersona(userId, draft)` fire-and-forget function added to draftService: deduplicates content against existing posts, checks quota, runs `analyzePersona()` on the published post, calls `mergePersonaAnalysis()`, appends to `scrapedPosts` + `postMetadata` (source: `'add-posts'`), increments `personaVersion`. Only fires when draft content ≥100 chars and persona exists. Route returns `{ message, draft, personaFeed: 'queued' }`. ✅ DONE
- [x] **#32 — Add "My Posts" link to Navbar** (`components/layout/Navbar.tsx` MODIFY): Added `<Link href="/dashboard/posts">My Posts</Link>` to the nav between "History" and "My Profile". (Note: app has no Sidebar component — nav lives in Navbar.) ✅ DONE

---

## Phase F — Twitter/X Multi-Platform Support ✅ COMPLETE (2026-02-26)

*Depends on Phase B (#12 platform config, #14 shared types). Can run in parallel with Phase D/E.*

- [x] **#33 — Add `platform` field to ContentSuggestion / ISuggestion** (`models/ContentSuggestion.ts` MODIFY, `packages/shared-types/src/index.ts` MODIFY): Added `platform: { type: String, enum: ['linkedin', 'twitter'], default: 'linkedin' }` to `suggestionItemSchema`. Extended format enum to include Twitter formats: `tweet`, `thread`, `quote-tweet`, `image-tweet`. Added `threadContent` nested schema (array of `{ tweetIndex, content, charCount }`, `_id: false`). `ISuggestion` already had `platform?` and `threadContent?` from Phase B #14. ✅ DONE
- [x] **#34 — Add platform section to content generator prompt** (`agents/contentGenerator.ts` MODIFY): Added `buildPlatformSection(platforms?: string[])` — returns `""` for LinkedIn-only default; otherwise builds `## PLATFORM REQUIREMENTS` section using `getPlatformConfig()` per platform (maxChars, formats, hashtag strategy, best practices, thread support note). Updated `generateContentIdeas()` signature to accept `platforms?: string[]`. Added `effectivePlatforms` resolution (prefers explicit `platforms` arg over `context.platforms`). Dynamic prompt intro adapts to platform: "LinkedIn", "Twitter/X", or "LinkedIn + Twitter/X". Updated `SuggestionSchema` to include `platform: z.enum(["linkedin","twitter"]).default("linkedin")`. Extended `format` enum to include Twitter formats. Simplified retry prompt now includes `"platform":"linkedin"` field. ✅ DONE
- [x] **#35 — Add platform selector to GenerateOptionsPanel** (`components/suggestions/GenerateOptionsPanel.tsx` MODIFY): Added `PLATFORM_OPTIONS` array (linkedin / twitter / both). Added `selectedPlatform: SuggestionPlatform | "both"` state (default: `"linkedin"`). Added `PlatformSelector` sub-component (3 toggle buttons, Twitter/both contextual notes). Platform selector rendered on: mode selection screen, Profile mode confirm screen, Topic focus confirm screen, and inside chat-refined mode (persists across mode switches). `resolvePlatforms()` converts `"both"` → `["linkedin","twitter"]`. `handleGenerate()` passes resolved `platforms` array into `IGenerateContextOptions`. ✅ DONE
- [x] **#36 — Build Twitter thread editor in PostEditorPane** (`components/editor/PostEditorPane.tsx` MODIFY): Added `TwitterThreadEditor` sub-component — numbered tweet boxes (Tweet 1/N), each with per-tweet 280-char counter (color-coded), `✕ Remove` button, `+ Add Tweet` button, dashed "add another tweet" bottom button. `threadMode` state auto-activates when `isTwitter && charCount > 280`. `useEffect` syncs mode when AI applies content. `handleThreadChange()` merges tweet array back to parent via `tweets.join("\n\n")`. Toggle button switches between "Thread editor mode" and "Single tweet mode". Thread editor scrollable (max-h-[480px]). ✅ DONE
- [x] **#37 — Add platform badge to SuggestionCard** (`components/suggestions/SuggestionCard.tsx` MODIFY): In the card header flex row, inserted a conditional platform chip before the format badge: `{suggestion.platform === "twitter" && <span className="...bg-sky-100 text-sky-700...">𝕏 Twitter</span>}`. LinkedIn badge intentionally omitted to keep the UI clean for the majority of cards. Header div updated to `flex-wrap` to handle both badges. ✅ DONE
- [x] **#38 — Update generate route to accept `platforms`** (`routes/suggestions.ts` MODIFY, `agents/mastra.ts` MODIFY): Added `platforms: z.array(z.enum(['linkedin','twitter'])).max(2).optional()` to both `generateContextSchema` (as `context.platforms`) and to `generateSchema` as a top-level field. Route resolves `effectivePlatforms = body.platforms ?? body.context?.platforms`. Passes to `runContentPipelineWithRetry({ ..., platforms: effectivePlatforms })`. `PipelineInput` in `mastra.ts` gains `platforms?: string[]` field, passed to `generateContentIdeas()`. `ContentSuggestion` save now persists `platform: idea.platform ?? "linkedin"` per suggestion item. ✅ DONE

---

## Phase G — Admin Dashboard ✅ COMPLETE (2026-02-26)

*Depends on Phase B (#8 AdminAuditLog, #9 User.role). Can run in parallel with Phase C/D/E.*

- [x] **#39 — Create admin seed service** (`services/adminSeed.ts` CREATE, `index.ts` MODIFY): `seedAdminAccount()` — checks if any user with `role: 'admin'` exists; if yes, logs and returns. If no: generates `placeholderEmail = admin-${Date.now()}@placeholder.local`, generates `setupToken = crypto.randomBytes(32).toString('hex')`, creates User with `role: 'admin', requiresSetup: true, hashed random password`. Stores `setupToken` in `SystemConfig` with key `admin_setup_token`. Logs admin ID + setup instructions to console. Called inside `start()` in `index.ts` after `seedDefaultTokenLimit()`. Exported `ADMIN_CONFIG_KEYS.SETUP_TOKEN` constant. ✅ DONE
- [x] **#40 — Create admin API routes** (`routes/admin.ts` CREATE, `index.ts` MODIFY): All routes behind `router.use(authenticate, requireAdmin)`. Implemented: `GET /api/admin/users` (paginated, searchable), `GET /api/admin/users/:id` (full user + persona + recent logs), `PATCH /api/admin/users/:id` (name/role/tokenLimit), `GET /api/admin/analytics/overview` (totalUsers, activeThisWeek, activeThisMonth, pendingRequests, totalSuggestions, totalTokensUsed, recentActivity), `GET /api/admin/analytics/tokens-over-time` (30-day daily aggregation), `GET /api/admin/config`, `PATCH /api/admin/config/:key` (with AdminAuditLog), `GET /api/admin/profile`, `PATCH /api/admin/profile` (name/email/password, bcrypt hash via pre-save hook). Setup endpoint separated into `adminSetupRouter` (authenticate only, no requireAdmin). Registered: `app.use('/api/admin', adminRoutes)` + `app.use('/api/admin-setup', adminSetupRouter)`. Also added `role` claim to JWT in `routes/auth.ts` (register + login + refresh). ✅ DONE
- [x] **#41 — Move token admin endpoints to admin routes** (`routes/tokenUsage.ts` MODIFY, `routes/admin.ts` MODIFY): Removed `GET /api/tokens/admin/requests` and `PATCH /api/tokens/admin/requests/:id` from `routes/tokenUsage.ts` (also removed `requireAdmin` import). Added them to `routes/admin.ts` as `GET /api/admin/token-requests` and `PATCH /api/admin/token-requests/:id` — now protected by `requireAdmin` at router level. Both approve/reject actions log to `AdminAuditLog`. ✅ DONE
- [x] **#42 + #48 — Build admin layout + Next.js middleware** (`app/admin/layout.tsx` CREATE, `middleware.ts` MODIFY): Admin layout with sidebar: Overview / Users / Token Requests / Analytics / Profile. Active link highlighting. Back-to-app link. Admin badge in top bar. In `middleware.ts`: added `/admin/setup` guard (any authenticated user — bypasses admin role check), then `/admin` guard (decodes JWT payload client-side to check `role === 'admin'`, redirects non-admins to `/dashboard`). Added `decodeJwtPayload()` helper using `atob()` (Edge runtime compatible). ✅ DONE
- [x] **#43 — Build admin overview page** (`app/admin/page.tsx` CREATE): Four stat cards (Total Users, Active This Week, Total Tokens Used, Pending Requests). Recent Activity timeline (last 10 token log entries). Pending Token Requests queue with inline Approve/Reject actions — approve includes new limit input. All data from `GET /api/admin/analytics/overview` + `GET /api/admin/token-requests?status=pending`. ✅ DONE
- [x] **#44 — Build user management page** (`app/admin/users/page.tsx` + `app/admin/users/[id]/page.tsx` CREATE): Paginated table with search. Columns: User (name/email), Role (badge), Token Usage (mini bar + numbers), Joined. Row → detail page. Detail page: edit name/role/tokenLimit, token bar with percentage, last 20 ops list, persona summary (industry, goals, pillars, interview status). ✅ DONE
- [x] **#45 — Build token requests page** (`app/admin/token-requests/page.tsx` CREATE): Status tabs (All/Pending/Approved/Rejected). Paginated table: User email, Current Usage, Message/Reason, Status chip, Submitted date. Pending rows have inline new-limit input + Approve + Reject buttons. Approved rows show final limit. ✅ DONE
- [x] **#46 — Build analytics page** (`app/admin/analytics/page.tsx` CREATE): Six stat blocks (total users, active this week/month, total tokens used, total suggestions, avg tokens/op). Inline bar chart for daily token usage over 30 days (no external charting library). Pending requests callout with link to token-requests page. ✅ DONE
- [x] **#47 — Build admin profile and setup pages** (`app/admin/profile/page.tsx` + `app/admin/setup/page.tsx` CREATE): Profile page: name, email, current/new/confirm password fields. Validates match + min 12 chars client-side. Setup page: 4 fields (setupToken, name optional, email, password, confirm). Validates match + min 12 chars. On success shows completion screen + auto-redirects to `/login` after 3s. Setup page accessible via `/admin/setup` to any authenticated user (bypasses requireAdmin in middleware). ✅ DONE
- [x] **#49 — Add admin API client** (`apps/web/src/lib/api.ts` MODIFY): Added `adminApi` object with: `getUsers(page?, limit?, search?)`, `getUser(id)`, `updateUser(id, body)`, `getTokenRequests(status?, page?)`, `updateTokenRequest(id, body)`, `getAnalyticsOverview()`, `getTokensOverTime()`, `getConfig()`, `updateConfig(key, value)`, `getProfile()`, `updateProfile(body)`, `setup(body)`. Added local interfaces: `IAdminUserSummary`, `IAdminTokenRequest`, `IAdminAnalyticsOverview`, `IAdminConfig`. ✅ DONE
