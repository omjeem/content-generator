...
- [ ] **#15 — Create feedback API routes** (`routes/feedback.ts` CREATE, `index.ts` MODIFY): Implement three endpoints: `POST /api/suggestions/:setId/feedback` (validate body with zod — `suggestionIndex`, `rating?`, `action`, `feedbackText?`; look up suggestion set to get snapshot; create `SuggestionFeedback` doc; fire-and-forget `processFeedback`; return 201), `GET /api/suggestions/:setId/feedback` (returns all feedback for a set by this user), `GET /api/feedback/summary` (aggregates preferred topics, format distribution, avg rating for the dashboard). Register route in `index.ts` as `app.use('/api', feedbackRoutes)`. All routes require `authenticate` middleware. (§3.3)
- [ ] **#16 — Create feedback processor service** (`services/feedbackProcessor.ts` CREATE): `processFeedback(feedback)` — (1) if `feedbackText` is present, call a lightweight LLM parse to extract `parsedSignals` (topicRelevance, toneMatch, formatPreference, specificNotes) and update the doc via `$set`; (2) if feedback count for this user is divisible by 5, trigger `aggregateAndUpdatePersona(userId)` (from Phase H #50) as fire-and-forget. Keep this service decoupled — it reads from `SuggestionFeedback`, writes to `SuggestionFeedback.parsedSignals`, and calls the learning service. (§3.5)
- [ ] **#17 — Build feedback UI on SuggestionCard** (`components/suggestions/SuggestionCard.tsx` MODIFY): Add a collapsible feedback row at the bottom of each card (collapsed by default, expanded after first interaction). Row 1: quality rating buttons — ❤️ Loved / 👍 Good / 😐 Meh / 👎 Bad (toggle, one at a time). Row 2: action buttons — 📌 Save / ✏️ Write Draft / ❌ Dismiss. Optional: freetext input (shows when rating or action is selected). [Submit Feedback] button calls `feedbackApi.submit()`. Show "✓ Feedback saved" confirmation. State: `feedbackRating`, `feedbackAction`, `feedbackText`, `feedbackSubmitted`. Disable buttons after submission. (§3.4)
- [ ] **#18 — Add feedback API client** (`apps/web/src/lib/api.ts` MODIFY): Add `feedbackApi` object with: `submit(setId, body)` → `POST /api/suggestions/${setId}/feedback`, `getForSet(setId)` → `GET /api/suggestions/${setId}/feedback`, `getSummary()` → `GET /api/feedback/summary`. (§3.9)
- [ ] **#19 — Integrate feedback into content generator prompt** (`agents/contentGenerator.ts` MODIFY): Add `buildFeedbackSection(persona)` function — returns empty string if `persona.feedbackProfile?.totalFeedbackCount < 3`; otherwise builds a `## USER FEEDBACK SIGNALS` section with: topics they love (top 5), topics to avoid, format preferences (sorted by percentage, show those >10%), tone preference, average satisfaction score. Append this section to the main generation prompt between the persona summary and the trends section. (§3.8)
