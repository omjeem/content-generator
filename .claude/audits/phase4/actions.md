...
- [x] **#11 — Created rate limiting middleware** (`middleware/rateLimit.ts` CREATE). Three limiters keyed by `req.userId`: `generationLimiter` (5/min), `chatLimiter` (20/min), `aiCheckLimiter` (10/min). Applied to: `/generate`, `/generate-from-trends`, `/generate-from-topic` (generationLimiter); `/refine-context`, `/:id/chat` (chatLimiter); `/:id/ai-check`, `/:id/humanize` (aiCheckLimiter). Also applied `generationLimiter` to trends `GET /` and `GET /discover`. ✅ DONE

- [x] **#12 — Added pipeline step-level timeouts** (`agents/mastra.ts` MODIFY). Created `withTimeout<T>(promise, ms, label)` utility. Wrapped persona analysis (30s), trend research (15s), content generation (45s). Added overall 90s pipeline timeout wrapping `runContentPipeline` in `runContentPipelineWithRetry`. Uses `PIPELINE.STEP_TIMEOUTS` from constants. ✅ DONE

- [x] **#13 — Created circuit breaker utility** (`utils/circuitBreaker.ts` CREATE, `agents/mastra.ts` MODIFY). `CircuitBreaker` class with closed → open → half-open states. Config from `PIPELINE.CIRCUIT_BREAKER`: failureThreshold: 5, cooldownMs: 60_000, halfOpenRequests: 1. Singleton `pipelineBreaker` wraps entire retry loop in `runContentPipelineWithRetry()`. ✅ DONE

- [x] **#14 — Added exponential backoff to pipeline retry** (`agents/mastra.ts` MODIFY). Replaced `setTimeout(r, 1000 * attempt)` with `Math.pow(2, attempt - 1) * 1000`: attempt 1 = 1s, attempt 2 = 2s. Uses `PIPELINE.MAX_RETRY_ATTEMPTS` constant. ✅ DONE

- [x] **#15 — Added trend deduplication across sources** (`services/trends.ts` MODIFY). Rewrote `deduplicateAndRank()` with improved dedup: `normalizeTitleKey()` (lowercase, strip non-alnum, sort words), groups by normalized key, keeps highest `sourceQuality()` winner (Tavily 4 > HN-high 3 > RSS 2 > HN-low 1 > Google News 0). Stores alternate URLs from duplicates on winning `RawTrendItem.alternateUrls`. ✅ DONE

- [x] **#16 — Persistent trend cache (MongoDB L2)** (`models/TrendCache.ts` CREATE, `services/trendCache.ts` MODIFY). Created `TrendCache` Mongoose model with TTL index (expires: 1800s). Rewrote `trendCache.ts` for two-tier caching: L1 in-memory Map (5-min TTL via `CACHE.L1_CACHE_TTL_MS`), L2 MongoDB (30-min TTL via auto-expiry). Lookup: L1 → L2 → miss. Store: both L1 + L2. Updated callers in `trendResearch.ts` with `await`. ✅ DONE
