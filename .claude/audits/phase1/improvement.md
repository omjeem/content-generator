...
function validateFormatDistribution(
  ...
  const formatCounts = ideas.ideas.reduce(
    (acc, i) => {
  ...
  if (
    preferredMix === "more-carousels" &&
    (formatCounts["carousel"] ?? 0) < ideas.ideas.length * 0.5
  ) {
...
const trendCache = new Map<
  string,
  { data: RawTrendItem[]; expiresAt: number }
>();
...
function buildHistoryPrompt(messages: IMessageDocument[]): string {
  ...
  const olderSummary = `[Previous conversation summary: The user discussed ${extractTopics(
    older,
  ).join(", ")}. Key answers provided: ${extractKeyAnswers(older)}]`;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        console.warn(
          `[contentGen] Attempt ${attempt} failed, retrying with simplified prompt...`,
        );
...
const addPostsSchema = z.object({
  posts: z
    .array(z.string().min(30, "Each post must be at least 30 characters"))
    .min(1, "Provide at least one post")
    .max(20, "Maximum 20 posts per batch"),
    ...
    try {
      ...
      if (reanalyze) {
        if (analysisMode === "full") {
          ...
          const { analysis: deltaAnalysis, usage } =
            await analyzePersona(newPosts);
...
export function mergePersonaAnalysis(
  ...
  return {
    ...
    topics: deduplicateStrings([...existing.topics, ...delta.topics]),
...
export function computePersonaDiff(
  ...
  if (old.writingStyle !== updated.writingStyle) {
    diff.writingStyle = {
      before: old.writingStyle,
      after: updated.writingStyle,
    };
  }
  ...
  const addedFormats = updated.postFormats.filter(
    (f) => !old.postFormats.includes(f),
  );
...
function normalizeForDedup(text: string): string {
  return text.toLowerCase().replace(/\s+/g, " ").trim().slice(0, 200); // compare first 200 chars to handle minor edits
}
```
...
export const personaApi = {
  ...
  getPosts: () =>
    request<{ posts: IPostMetadata[]; totalPosts: number }>(
      "/api/persona/posts",
    ),
};
```
...
<Card>
  <CardContent className="p-5">
    ...
    <p className="text-xs text-gray-500 mb-4">
      Add new LinkedIn posts to refine your persona. Your writing style, tone,
      and topics will be updated based on the combined corpus.
    </p>
    <PostInputCards
...
const analyzeSchema = z
  .object({
    ...
    manualPosts: z
      .string()
      .min(30, "Please provide at least one post")
      .optional(),
