...
export function PendingChangesCard({ changes, onApply, onDiscard, applying }: PendingChangesCardProps) {
  const changedFields = Object.entries(changes).filter(([, v]) => v !== undefined && v !== null)
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              The AI is suggesting updates to your profile. Review and apply or discard.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2 mb-4">
          {changedFields.map(([key, value]) => (
            <div key={key} className="rounded-lg bg-white border border-amber-100 px-3 py-2">
