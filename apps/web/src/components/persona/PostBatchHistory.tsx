...
export function PostBatchHistory({
  ...
  const [batches, setBatches] = useState<IPostBatchMetadata[]>(initialBatches ?? [])
  const [totalPostsAnalyzed, setTotalPostsAnalyzed] = useState(initialTotal ?? 0)
  ...
  useEffect(() => {
    ...
    personaApi.getPosts()
      ...
      .catch((err) => {
        setError(err instanceof ApiError ? err.message : 'Failed to load batch history')
      })
  ...
  if (error) {
    return (
      <div className={`rounded-lg bg-red-50 border border-red-200 px-3 py-2 text-xs text-red-700 ${className}`}>
        {error}
      </div>
    )
  }
