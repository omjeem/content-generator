...
export function GenerateOptionsPanel({ disabled, onGenerate, onCancel }: GenerateOptionsPanelProps) {
  ...
  useEffect(() => {
    if (mode === 'chat-refined' && chatMessages.length === 0) {
      setChatMessages([{
        role: 'assistant',
        content: "Hi! I'll help you refine the focus for your next batch of content ideas. What topic or niche would you like to explore — or is there a specific audience or goal you have in mind?",
  ...
  const handleGenerate = () => {
    if (mode === 'profile') {
      ...
      onGenerate({
        ...
        platformGoal: refinedContext.platformGoal as IGenerateContextOptions['platformGoal'],
      })
    }
  }
    ...
    return (
      <Card>
        <CardContent className="p-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-1 text-center">How do you want to generate ideas?</h2>
          <p className="text-sm text-gray-500 text-center mb-6">Choose how to tailor this batch of content ideas.</p>
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <h2 className="text-lg font-semibold mb-1">Generate from Your Profile</h2>
          <p className="text-sm text-gray-500 mb-6 max-w-sm mx-auto">
            We&apos;ll use your full persona — writing style, goals, content pillars, and trending topics in your niche.
          </p>
          <div className="flex gap-3 justify-center">
            <Button variant="outline" onClick={() => setMode(null)}>← Back</Button>
            <Button onClick={handleGenerate} disabled={disabled} className="min-w-[200px]">
    ...
    return (
      <Card>
        <CardContent className="p-8">
          ...
          <h2 className="text-lg font-semibold mb-1 text-center">Focus on a Specific Topic</h2>
          <p className="text-sm text-gray-500 text-center mb-6 max-w-sm mx-auto">
            All ideas in this batch will focus on the topic or niche you specify.
          </p>
    ...
    <Card>
      <CardContent className="p-0">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          <div>
            <h2 className="text-base font-semibold text-gray-900">💬 Refine with AI</h2>
            <p className="text-xs text-gray-500">Chat to clarify your angle before generating</p>
          </div>
          <button onClick={() => setMode(null)} className="text-sm text-gray-400 hover:text-gray-600">← Back</button>
        </div>
        ...
        {refinedContext?.summary && (
          <div className="mx-4 mb-3 rounded-lg bg-green-50 border border-green-200 px-4 py-3">
            <p className="text-xs font-semibold text-green-700 mb-0.5">✓ Context gathered</p>
