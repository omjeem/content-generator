...
import type { ITokenUsageSummary, ITokenUsageLog, OperationType } from '@repo/shared-types'
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Track your AI token consumption across all operations</p>
      </div>
      ...
      {summary && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-lg font-bold ${getPercentTextColor(summary.percentUsed)}`}>
              {summary.percentUsed}%
            </span>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-between mt-1.5">
              ...
              <span className="text-xs text-gray-500">{summary.tokenLimit.toLocaleString()} tokens</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="rounded-lg bg-gray-50 p-4 text-center">
              <div className={`text-2xl font-bold ${summary.tokensRemaining === 0 ? 'text-red-600' : 'text-gray-900'}`}>
          ...
          {summary.percentUsed >= 100 && (
            <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4">
              ...
              <div>
                <p className="text-sm font-semibold text-red-700">Token quota exceeded</p>
                <p className="text-sm text-red-600 mt-0.5">
                  You have used all available tokens. AI operations are currently blocked.
          ...
          {summary.percentUsed >= 80 && summary.percentUsed < 100 && (
            <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-700">Approaching token limit</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{total.toLocaleString()} total operations</p>
          </div>
          {loading && (
        ...
        {logs.length === 0 ? (
          <div className="py-12 text-center text-sm text-gray-400">
            No operations recorded yet. Start using the AI features to see usage here.
          </div>
        ) : (
          <>
                  ...
                  {logs.map((log) => (
                    <tr key={log._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-3.5">
                        ...
                        <span className="ml-2 text-xs text-gray-400">{log.agent}</span>
                      </td>
