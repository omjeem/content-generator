...
const addPostsSchema = z.object({
  postsArray: z
    .array(z.string().min(10, "Each post must be at least 10 characters"))
    ...
    try {
      ...
      if (scrapingBlocked) {
        res.status(422).json({
          ...
          fallback:
            "Please paste your LinkedIn posts manually using the manualPosts or postsArray field.",
        });
        return;
      }
      ...
      if (posts.length === 0) {
        res
          .status(400)
          .json({
            error: "No posts found to analyze. Please provide more content.",
          });
        return;
      }
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!existing) {
        res.status(404).json({
          error:
            "No existing persona found. Please run POST /api/persona/analyze first.",
        });
        return;
      }

      // Deduplicate new posts against existing ones
      const uniqueNewPosts = deduplicatePosts(
        existing.scrapedPosts ?? [],
        body.postsArray,
      );
      if (uniqueNewPosts.length === 0) {
        res.status(400).json({
          error:
            "All provided posts are duplicates of existing posts. No new posts to add.",
      ...
      const updatedPersona = await UserPersona.findOneAndUpdate(
        { userId },
        updateQuery,
        { new: true },
      );
      ...
      const diff = updatedPersona
        ? computePersonaDiff(snapshot, updatedPersona)
        : null;
