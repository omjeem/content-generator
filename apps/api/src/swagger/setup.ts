...
- **Header**: \`Authorization: Bearer <token>\`
      ...
      schemas: {
        UserPersona: {
          ...
          properties: {
            ...
            writingStyle: { type: 'string', example: 'conversational, story-driven' },
            tone: { type: 'string', example: 'professional yet approachable' },
            topics: { type: 'array', items: { type: 'string' }, example: ['AI', 'leadership', 'startups'] },
            postFormats: { type: 'array', items: { type: 'string' }, example: ['carousel', 'text-post'] },
            goals: { type: 'string' },
        ...
        Suggestion: {
          ...
          properties: {
            ...
            angle: { type: 'string', example: 'Why async-first teams outperform sync-heavy ones' },
            format: {
              type: 'string',
              enum: ['carousel', 'text-post', 'poll', 'video-script', 'list'],
            },
            hook: { type: 'string', example: 'We hired 12 people without a single Zoom call.' },
            whyItFits: { type: 'string', example: 'Matches your data-driven tone and leadership pillar' },
          },
        },
        ContentSuggestion: {
          type: 'object',
    ...
    tags: [
      ...
      { name: 'Onboarding', description: 'Interview agent with persistent memory (Agent 2)' },
      { name: 'Trends', description: 'Real-time trend research (Agent 3)' },
      { name: 'Suggestions', description: 'AI content idea generation (Agent 4 + Orchestrator)' },
    ],
  },
  apis: ['./src/routes/*.ts'],
}
